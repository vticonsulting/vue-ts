...
<template>
  <form @click.prevent="save" class="flex-1 lg:col-span-9">
    <SectionWrapper>
      <SectionHeader>{{ t('volunteer_eligibility') }}</SectionHeader>

      <!-- Volunteer Eligibility -->
      <fieldset class="mt-8">
        <h3 class="text-lg font-medium">Volunteer Eligibility</h3>- Signed Agreement on:
        <button
          type="button"
          class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
        >View / Print Agreement</button>

        <OField class="mt-4">
          <OSwitch>Has church volunteer approval</OSwitch>
        </OField>
      </fieldset>
